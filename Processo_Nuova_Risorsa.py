...
# Visualizziamo i gruppi Azure (inclusi grp_foorban e grp_salesforce)
...
# Mostriamo anche il contenuto attuale del campo InserimentoGruppo (se presente)
...
# --- Preparazione CSV Profilazione (ex O365) ---
# existing_o365 è la lista dei gruppi O365 di default (gia' splittata)
existing_o365 = list(o365_groups) # copia

# gruppi presi dall'InserimentoGruppo del primo CSV (row_ut)
inser_gr_raw = inserimento_gruppo or ""
inser_list = [g.strip() for g in str(inser_gr_raw).split(";") if g.strip()]

# unisci evitando duplicati, preservando ordine (prima existing_o365 poi eventuali inserimenti nuovi)
...
# costruzione riga Profilazione (manteniamo header HEADER_UTENTE)
...
# --- preparazione buffer per download ---
# CSV Utente
...
# CSV Profilazione (ex O365)
buf_profilazione = io.StringIO()
w3 = csv.writer(buf_profilazione, quoting=csv.QUOTE_NONE, escapechar="\\")
quoted_row_profilazione = auto_quote(
row_profilazione,
quotechar='"',
predicate=lambda s: ' ' in s
)
w3.writerow(HEADER_UTENTE)
w3.writerow(quoted_row_profilazione)
buf_profilazione.seek(0)
...
data=buf_profilazione.getvalue(),
